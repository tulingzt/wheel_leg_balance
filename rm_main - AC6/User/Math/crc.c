...
void crc16_set_checksum(uint8_t *pdata, uint16_t length)
{
    ...
    pdata[length-1] = (uint8_t)((crc16 >> 8) & 0x00ff);
}

static uint16_t crc_ccitt_get_byte(uint16_t crc, const uint8_t data)
{
...
{
    uint16_t tmp = crc_init;
    while (length--)
        tmp = crc_ccitt_get_byte(tmp, *pdata++);
    return tmp;
}
